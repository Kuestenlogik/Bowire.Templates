...
- [x] **2026-09-17** — `--BowireSdkVersion` defaults to the range `2.*` instead of a pin, and the ProjectOnly/Minimal output finally honours the parameter at all (it carried a literal beside the token and silently ignored what you passed). Under SemVer the whole major is the backwards-compatible set, so `2.*` is as wide as a default can be without ever handing out an incompatible SDK. A pinned default had to be kept current by hand, and was not: it sat at `1.6.0` for months, and the cascade's merged bump to `2.7.0` was later lost to a rewrite of `main`. A range resolves at restore time and still packs a concrete floor, so nothing downstream changes. See [SDK version](docs/BowirePluginTemplate.md#sdk-version).
...
- [x] **Bowire-SDK-version auto-bump** — dropped rather than built. The floating default (`2.*`, 2026-09-17) picks up every backwards-compatible release at restore time without anyone writing a version down. What is left is one manual move per major (`2.*` → `3.*`), which a cron job watching nuget.org for a newer version would not have caught anyway — the release cascade deliberately declines to overwrite a range, whatever its width.
